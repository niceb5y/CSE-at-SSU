...
int main(int argc, char *argv[])
{
  ...
  if ((fdout = open(argv[2], O_RDWR | O_CREAT | O_TRUNC,
    ...
    serror("cant create file for writing");
  ...
  if ((inp = mmap(0, statbuf.st_size, PROT_READ, MAP_SHARED, fdin, 0)) ==
      (caddr_t)-1)
    serror("mmap error for input");

  if ((outp = mmap(0, statbuf.st_size, PROT_READ | PROT_WRITE, MAP_SHARED,
                   fdout, 0)) == (caddr_t)-1)
